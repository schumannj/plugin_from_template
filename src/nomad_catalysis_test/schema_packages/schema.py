...
def populate_reactivity_info(archive, self, logger):
    ...
    for ref_attr, reaction_attr in quantities_results_mapping.items():
        ...
        if value is not None:
            print(value)
            try:
                set_nested_attr(archive.results.properties.catalytic.reaction,
                    reaction_attr, value)
            except ValueError:
                set_nested_attr(archive.results.properties.catalytic.reaction,
                        reaction_attr, [value])
            except:
                logger.warn('Something else went wrong when trying setattr for reaction')
...
class CatalyticReactionCleanData(CatalyticReaction_core, PlotSection, Schema):
    """
    This schema is originally adapted to map the data of the clean Oxidation dataset
    (JACS, https://doi.org/10.1021/jacs.2c11117) The descriptions in the quantities
    represent the instructions given to the user who manually curated the data. The
    schema has since been extended to match other, similar datasets, with multiple
    products.
    """
    m_def = Section(
        label='Catalytic Reaction (filled by Clean Data Table)',
        a_eln=ELNAnnotation(properties=dict(
            order= ['name','data_file', 'reaction_name','reaction_class',
    ...
    results.a_eln = ELNAnnotation(label='Reaction Results') #does not work atm
    ...
    def normalize(self, archive, logger):
        ...
        for col in data.columns:
            ...
            if col_split[0] == "x_r":  # conversion, based on reactant detection
                ...
                try:
                    ...
                    conversion = Reactant_data(
                        ...
                        conversion_type='reactant-based conversion',
                        conversion_reactant_based=np.nan_to_num(data[col]),
                        gas_concentration_in=np.nan_to_num(data['x '+col_split[1]])*100)
                except:
            ...
            if col_split[0] == "y":  # concentration out
                if col_split[1] in reagent_names:
                    conversion = Reactant_data(
                        ...
                        conversion=np.nan_to_num((1-(data[col]/data['x '+col_split[1]
                                                                    +' (%)']))*100))
                    conversions.append(conversion)
                else:
            ...
            if col_split[0] == "S_p":  # selectivity
                product = Product_data(
                    name=col_split[1],
                    selectivity=np.nan_to_num(data[col]))
        ...
        if feed.set_total_flow_rate is not None and (
            reactor_filling.catalyst_mass is not None):
            feed.weight_hourly_space_velocity = (feed.set_total_flow_rate
        ...
        populate_reactivity_info(archive, self, logger)
        ...
        #     (archive.results.properties.catalytic.reaction.
        #      reaction_conditions.temperature) = cat_data.temperature
